...
const Styles = cvx({
  variants: {
    style: {
      aside:
        "m-0 h-dvh max-h-dvh bg-background overflow-hidden [transition:all_0.5s_ease] w-0 focus-visible:outline-0 top-0 bottom-0 md:sticky md:top-[calc(var(--navbar)*1)] max-md:data-[state=closed]:opacity-0 md:pt-8",
      hgroup: "flex flex-row items-center justify-between h-[--navbar] mb-4 md:hidden md:sr-only",
      nav: "relative flex items-start justify-start flex-col flex-nowrap size-full pl-4 pr-1.5 max-md:pt-0 overflow-y-auto overflow-x-hidden webkit-scrollbar",
      overlay:
