    ...
    <div>
      <section className="relative rounded-md border bg-background">
        <ScrollArea
          ...
          classNames={{
            content: "max-h-full gap-4 p-4 flex data-[overflow=y]:flex-col data-[overflow=x]:flex-row data-[overflow=y]:h-80 data-[overflow=y]:w-64 data-[overflow=y]:overflow-y-auto data-[overflow=x]:h-max data-[overflow=x]:w-80 data-[overflow=x]:overflow-x-auto",
            thumb: "hover:bg-muted-foreground peer-hover:bg-muted-foreground data-[scroll=active]:bg-color data-[overflow=x]:bottom-1 data-[overflow=y]:right-1",
          }}
        >
